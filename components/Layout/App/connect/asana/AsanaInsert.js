...
import { modules } from '../../../../../store/connect/asana/asana';
...
const Asana = () => {
  const connectType = 'asana';
  const dispatch = useDispatch();
  const router = useRouter();
  const {
    team, asana, user, connect,
  ...
  useEffect(() => {
    ...
    searcher.initialize({
      dispatch, document, team, user, asana, connectType, set: creators.setInputAsana,
    });
  ...
  useEffect(() => {
    template1.initialize({
      ...
      connectType: 'asana',
      modules,
      load: creators.getTeamsAsana,
      connect: [creators.postTeamsAsana, creators.putTeamsAsanaSetting],
      disconnect: creators.deleteAuthentications,
      set: creators.setInputAsana,
    }, false);
  }, []);
        ...
        <div className='tab-content'>
          <div className='tab-cont on'>
            <div className='detail-content'>
              <div className='info-wrap'>
                <img src={getPublicAssetPath('static/asana/ko/info/img_info.png')} alt="서비스 소개"></img>{/* [D] : static/커넥트명/언어코드/img_info.png */}
                <div className='info-box'>
                  <strong>asana</strong>
                  <p>asana은 Git 버전 컨트롤 시스템에 기반한 오픈소스 프로젝트를 위한 소셜 저장소입니다.<br/>asana을 잔디와 연동하게 되면, asana 브랜치의 Commit, Comments, Pull Request와 같은 다양한 변동 사항을 팀 내에서 메시지로 수신할 수 있습니다.</p>
                </div>
          ...
          <div className='tab-cont'>
              <div className='detail-content connect'>
                        ...
                        <div className='input-row'>
                          ...
                          <div className="select-box type-full">
                            ...
                            >
                              ...
                              {/* //[D] : 로딩 case */}
                              <span>{asana.input.selectedAuthentication}</span>
                            </a>
                                          ...
                                          if (data.status === 'created') {
                                            return (
                                              <li key={i}>
                                                ...
                                                <button type='button' className='btn-delete icon-ic-close' onClick={(e) => template1.disconnect(e, asana.input)}></button>
                                              </li>
                                            );
                                          }
                                        })
                ...
                <div className='connect-row-item'>
                  ...
                  <div className='content'>
                    <dl className='row'>
                      ...
                      <dd>
                        <div className='input-row'>
                          <Thumbnail state={asana} parent={template1} />
                          <Input type="text"
                                 className='input-type'
                                 onChange={(e) => template1.set('botName', e.target.value)}
                                 value={asana.input.botName}
                          ></Input>
                        </div>
                      </dd>
                    </dl>
                        ...
                        <div className='input-row'>
                          <div className="select-box type-full">
                            <a href="#none"
                              ...
                              <span>{asana.input.selectedTopic}</span>
                            </a>
                            <div className="select-list custom-select">
                              ...
                              {/* custom-select-wrap */}
                              {
                                asana.input.searchText === '' && (
                                  <div className='custom-select-wrap'>
                                    <dl className='option-wrap'>
                                      <dt className='tit'>토픽</dt>
                                      <dd>
                                        {
                                          asana.input.searchRooms.map((roomsData, roomsIndex) => (<Fragment key={roomsIndex}>
                                            {roomsData.seq
                              ...
                              {/* search-list-wrap 검샋결과가 있을 경우 폴더는 제외 */}
                              {
                                asana.input.searchText !== '' && (
                                  <div className='search-list-wrap'>
                                    {
                                      asana.input.searchFilters.length > 0
                                      && <>
                                        <p className='tit'>{asana.input.searchFilters.length}개의 결과가 있습니다.</p>
                                        <ul>
                                          {
                                            asana.input.searchFilters.map((roomData, roomIndex) => (
                                              <li key={roomIndex}>
                                    ...
                                    {asana.input.searchFilters.length === 0 && <p className='tit no-result'>결과가 없습니다.</p>}
                                  </div>
                                )
                              }
                              {/* search-list-wrap */}
                      ...
                      <dd>
                        <div className='input-row'>
                          <div className="select-box type-full">
                            ...
                            ><span>{asana.input.langText}</span></a>
                ...
                {/* disabled */}
                <button type='button' className='full-btn' onClick={(e) => template1.connect(e, asana)}>연동 추가하기</button>
              </>}
              </div>{/* //detail-wrapper */}
            </div>
          </div>
