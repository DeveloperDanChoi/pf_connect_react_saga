...
const GoogleCalendar = () => {
  ...
  useEffect(() => {
    // TODO: 여기서 해야하는가?
    if (googleCalendar.teamsGoogleCalendar.id > 0) {
      for (const item in googleCalendar.teamsGoogleCalendar) {
        template1.set(item, googleCalendar.teamsGoogleCalendar[item]);
      }
    }
    if (googleCalendar.teamsGoogleCalendar.id > 0 && team.teamsMembers.length === 0) {
  ...
  }, [googleCalendar.teamsGoogleCalendar, team.teamsMembers]);
