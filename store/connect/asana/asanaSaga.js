...
import { initialModules, modules } from './asana';
import {
  getTeamsBitbucket,
...
export const saga = (() => ({
   ...
   */
  * getTeamsToken(data, connectType = 'asana') {
    ...
    for (const key in result.data) {
      yield put(creators.setInputAsana({ key, value: result.data[key] }));
    }
  },
  /**
  ...
  * getTeamsAsana(data) {
    ...
    for (const member of resultMembers.data.records) {
      if (member.id === result.data.memberId) {
        yield put(creators.setInputAsana({ key: 'member', value: member }));
        break;
      }
    }

    // TODO: 제거 하거나 편집일 떄 input -> teams 데이터로 변경
    yield put(creators.setTeamsAsana(result.data));

    for (const key in result.data) {
      yield put(creators.setInputAsana({ key, value: result.data[key] }));
    }

    yield put(creators.setInputAsana({ key: 'createdAt', value: util.dateFormat(result.data.createdAt) }));
    yield put(creators.setInputAsana({ key: 'statusClss', value: converter.statusClss(result.data.status) }));
  },
  /**
  ...
  * putTeamsAsanaSetting(data) {
    ...
    if (result.status !== 200) {
      ...
      yield put(creators.getTeamsAsana({
   ...
   */
  * setInputAsana(data) {
    yield put(creators.setInputAsanaValue(data));
  },
  /**
   * 사용자 정의 데이터<br>
   * @param data
...
export default function* asanaSaga() {}
