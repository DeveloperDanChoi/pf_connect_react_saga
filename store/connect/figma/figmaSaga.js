...
import { initialModules, modules } from './figma';
import {
  getTeamsBitbucket,
...
export const saga = (() => ({
   ...
   */
  * getTeamsToken(data, connectType = 'figma') {
    ...
    for (const key in result.data) {
      yield put(creators.setInputFigma({ key, value: result.data[key] }));
    }
  },
  /**
  ...
  * getTeamsFigma(data) {
    ...
    for (const member of resultMembers.data.records) {
      if (member.id === result.data.memberId) {
        yield put(creators.setInputFigma({ key: 'member', value: member }));
        break;
      }
    }

    // TODO: 제거 하거나 편집일 떄 input -> teams 데이터로 변경
    yield put(creators.setTeamsFigma(result.data));

    for (const key in result.data) {
      yield put(creators.setInputFigma({ key, value: result.data[key] }));
    }

    yield put(creators.setInputFigma({ key: 'createdAt', value: util.dateFormat(result.data.createdAt) }));
    yield put(creators.setInputFigma({ key: 'statusClss', value: converter.statusClss(result.data.status) }));
  },
  /**
  ...
  * putTeamsFigmaSetting(data) {
    ...
    if (result.status !== 200) {
      ...
      yield put(creators.getTeamsFigma({
   ...
   */
  * setInputFigma(data) {
    yield put(creators.setInputFigmaValue(data));
  },
  /**
   * 사용자 정의 데이터<br>
   * @param data
...
export default function* figmaSaga() {}
